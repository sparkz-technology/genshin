...
import { GitPullRequestCreate, Settings as SettingsIcon, Signature } from "lucide-react";
...
const SettingsDialog: React.FC<ISettingsDialog> = ({ data, disabled }) => {
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <Tooltip delayDuration={200}>
          <TooltipTrigger asChild>
            <Button variant="outline" size="icon" onClick={() => setIsOpen(true)} disabled={disabled}>
        ...
        <DialogContent className="sm:max-w-[425px] md:max-w-[600px] lg:max-w-[800px] w-full">
          ...
          <motion.div initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <form onSubmit={formik.handleSubmit} className="space-y-4" key={isOpen ? "open" : "close"}>
              ...
              <Button type="submit" disabled={formik.isSubmitting || disabled}>
